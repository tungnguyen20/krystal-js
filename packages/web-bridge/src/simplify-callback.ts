...
import { createDataStream, createSubscription, DataStream, Subscription } from './subscription';
import { CallbackResult, isType, Omit, parseJson } from './utils';
    ...
    global[callbackName] = (data: CallbackResult<string>) => {
      ...
      if (isType<StreamEventResult>(result, 'event')) {
        ...
        !!handlers &&
          !!handlers.next &&
          handlers.next({
            result: result === null ? undefined : (parseJson(result) as T),
 ...
 */
export function simplifyCallback(global: any, { funcNameToWrap, ...restParams }: Params) {
